...
final class IdentifierDao extends BaseDao
{
    ...
    public function getFindSql(\BtcRelax\Dao\IdentifierSearchCriteria $vSearch)
    {
        ...
        $orderBy = ' CreateDate  ';
        $filter = '';
        if (!is_null($vSearch->getIdentityKey())) {
            $filter = $this->addToFilter($filter, sprintf('IdentityKey = \'%s\'', $vSearch->getIdentityKey()));
        }
        if (!is_null($vSearch->getIdentTypeCode())) {
            $filter = $this->addToFilter($filter, sprintf('IdentTypeCode = \'%s\'', $vSearch->getIdentTypeCode()));
        }
        if (!is_null($vSearch->getIdCustomer())) {
            $filter = $this->addToFilter($filter, sprintf('idCustomer = \'%s\'', $vSearch->getIdCustomer()));
        }
