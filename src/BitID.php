...
class BitID
{
    ...
    //private $_qrservice = "https://chart.googleapis.com/chart?cht=qr&chs=250x250&chl=";
    ...
    {
        return \sprintf("getimg/?uri=%s", $uri);
    }
    ...
    public function buildURI($callback, $nonce = null)
    {
        ...
        if (stripos($callback, 'https://')!==false) {
            ...
            $this->_secure = true;
        } elseif (stripos($callback, 'http://')!==false) {
            $this->_callback = str_replace('http://', '', $this->_callback);
            $this->_secure = false;
        }

        $result = $this->_scheme . '://' . $this->_callback . '?x=' . $this->_nonce . (!$this->_secure?'&u=1':'');
        //$result = \sprintf('%s://%s%s',$this->_scheme , $this->_callback , $this->_secure?'':'?u=1');
        return $result;
    }
     ...
     */
    public function _recoverPubKey($r, $s, $e, $recoveryFlags, $G)
    {
