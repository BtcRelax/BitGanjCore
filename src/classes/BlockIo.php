...
class BlockIo
{
    ...
    private function _request($path, $args = array(), $method = 'POST')
    {
        ...
        // Because SSL troubles
        $certificatesFilePath = dirname(__FILE__) . '/cacert.pem';
        if (file_exists($certificatesFilePath)) {
            curl_setopt($ch, CURLOPT_CAINFO, dirname(__FILE__) . '/cacert.pem');
            curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, true);
            BtcRelax\Log::general(sprintf("File with root certs found and loaded, from %s", $certificatesFilePath), BtcRelax\Log::INFO);
        } else {
            curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
            BtcRelax\Log::general(sprintf("File with root certs not found by path %s. Connection not secure!!!", $certificatesFilePath), BtcRelax\Log::WARN);
        }
...
class BlockKey
{
    ...
    public function calculateYWithX($x, $derEvenOrOddCode = null)
    {
        ...
        if (!$derEvenOrOddCode) {
            return $y;
        } elseif ($derEvenOrOddCode == '02') { // even
            $resY = null;
            ...
            return $resY;
        } elseif ($derEvenOrOddCode == '03') { // odd
            $resY = null;
