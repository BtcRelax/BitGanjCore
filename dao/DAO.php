...
class DAO extends Dao\BaseDao {
	...
	public function customerById($typeCode,$id) {
		$result = $this->_mysqli->query(sprintf("SELECT * FROM vwCustomerIds WHERE TypeCode = '%s'  AND Id = '%s' LIMIT 1 ", 
		$this->_mysqli->real_escape_string($typeCode),$this->_mysqli->real_escape_string($id)));
		if($result) {
			$row = $result->fetch_assoc();
