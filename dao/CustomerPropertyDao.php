<?php
	...
	public function findById($pIdCustomer)
	{
        ...
        {
            $vCloseQuery = \sprintf("UPDATE CustomerProperty SET `EndDate` = NOW() WHERE `idCustomer` = '%s' AND `PropertyTypeCode` = '%s'AND `EndDate` = 0;",
                    $customerProperty->getIdCustomer(), $customerProperty->getPropertyTypeCode());
            $vQuery = \sprintf("%s INSERT INTO CustomerProperty ( `idCustomer`, `PropertyTypeCode`, `PropertyValue`) VALUES ('%s','%s','%s');",$vCloseQuery,
                    $customerProperty->getIdCustomer(), $customerProperty->getPropertyTypeCode(), $customerProperty->getPropertyValue());
            $this->query($vQuery);
        }
