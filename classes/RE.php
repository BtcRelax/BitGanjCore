...
class RE implements IRE {
    ...
    {
        $result = null;
        if (!is_numeric($pPrice)) { throw new \LogicException('Price has incorrect value!'); }
        if ($pSourceCurrency===$pTargetCurrency) return $pPrice;
        ...
        if (!is_numeric($result)) { throw new \LogicException('Price undefined!'); }
        return $result;
    }
    ...
    public function getInvoiceInfoById($vParams) {
        ...
        } else { $result = ["error"=> "Invoice not found"]; }
        return $result;
    }
