...
class AM implements IAM
{
    ...
    {
        if (empty($this->_user)) {
            $this->_user = $this->getCurrentSession()->getValue('CurrentUser');
        }
        if (empty($this->_user)) {
    ...
    {
        if ($pUser === null) {
            $this->_currentSession->clearValue('CurrentUser');
        } else {
            $this->_currentSession->setValue('CurrentUser', $pUser);
        }
        $this->_user = $pUser;
    }
    ...
    public function beginAuthenticate($pAuthType, $pParams = null)
    {
        ...
        if (!$isActiveAuth) {
            ...
            $vCurrentIdent->init();
            $vResult =  $vCurrentIdent->processAuth($pParams);
            if ($vResult) {
                $newUser = new \BtcRelax\Model\User();
                ...
                $this->setLastError($vCurrentIdent->getLastError());
            }
        } else {
            $this->setLastError("Authentification process already started!");
        }
        return $vResult;
    }
    ...
    public function checkAuth($vParams = null)
    {
        ...
        if (!\is_null($isActiveAuth)) {
            $vIdent = $vSession->getValue($isActiveAuth);
    ...
    public function isUserHasRight($pRightCode, $pUser = null)
    {
        if (empty($pRightCode)) {
            ...
            foreach ($vUserRights as $right) {
                if ($right->getRightCode() === $pRightCode) {
                    return true;
                }
            }
        }
    ...
    public function NotifyRoot(string $vMessage, string $vEvent)
    {
        ...
        if ($vIsEnabled === true) {
            ...
            if (false !== $vNotificator) {
                ...
                $this->setLastError(\sprintf("Error getting notificator for user id: %s", $pUserId));
            }
        }
    }
    ...
    public function createNewUser()
    {
        ...
        if ($ident != false) {
            if (IS_FREE_REGISTER) {
                ...
                $this->setLastError("Не, розслабься. Джа не велел тебя пускать, и я тебя не пропущу. Но, если ты так считаешь что я ошибся, то обратись к Джа, возможно он тебя пропустит.");
            }
        } else {
            $this->setLastError("Hasnt any identification for register");
        }
        return $result;
    }
...
//    }
    ...
    public function getUserNotificatorByUserId($pUserId)
    {
        ...
        if ($vUser instanceof \BtcRelax\Model\User) {
            ...
            $this->setLastError(\sprintf("User id:%s not found, when try to get notificator", $pUserId));
        }
        return $result;
    }
    ...
    public function SignIn(\BtcRelax\Model\Identicator $pIdent)
    {
        ...
        if (\FALSE !== $vIdentRow) {
            ...
            \BtcRelax\Log::general(\sprintf('User not found by identifier:%s', $pIdent), \BtcRelax\Log::INFO);
        }
        $this->setSessionState();
        return $result;
    }
    ...
    public function getUserInfoById($vParams = null)
    {
        if (empty($vParams)) {
            ...
            $vParams = ["id" => $vCurrentUser->getIdCustomer()];
        }
