...
class Order
{
    ...
    {
        if (\count($this->pBookmarks) === 1) {
            $result = $this->pBookmarks[0];
        }
        return $result;
    }
    ...
    public function setOrderHash($pOrderHash)
    {
    ...
    public function setCreateDate(\DateTime $pCreateDate)
    {
    ...
    public function __toString()
    {
        ...
        if (!empty($this->pIdOrder)) {
            $result .= \sprintf("OrderId:%s|", $this->pIdOrder);
        }
        if (!empty($this->pState)) {
            $result .= sprintf("State:%s|", $this->pState);
        }
        return $result;
    }
}
