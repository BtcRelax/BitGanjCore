...
final class Core {
    ...
    private function init( bool $vIsExceptionHandler = true ) {
                try {                     
                    ...
                    if ($vIsExceptionHandler) { \set_exception_handler([$this, 'handleException']);}
                    config::init();
    ...
    public static function createOM(): \BtcRelax\OM { return new OM(); }

    public static function createRE(): \BtcRelax\RE { return new RE(); }
        
    public static function createPM(): \BtcRelax\PM { return new PM(); }
    ...
    public function getCurrentSession(): \BtcRelax\SecureSession {
            ...
            { return $this->current_session; } else { \BtcRelax\Log::general("Incorrect session instance inside core!", \BtcRelax\Log::FATAL); }
    }
    ...
    public function loadClass($name):bool {
            ...
            if (array_key_exists($name, self::$CLASSES))
            {
                ...
                if (!$this->tryToAutoload($name)) { require_once __DIR__ . '/vendor/autoload.php'; } else {return true; } }
...
//			}
    ...
    function tryToAutoload($f) {       
        $result = false;  $base = dirname(__FILE__) . "/";
        ...
        if (file_exists($classFile)) {
            require_once $classFile;  $result = true;
        }
        $utilFile = $base . "classes/util/" . $f . ".php";
        if (file_exists($utilFile)) {
            require_once $utilFile;  $result = true;
        }
        $fpath = explode("\\", $f);
